...
API_SECRET_KEY = os.getenv("API_SECRET_KEY", "change-secret-key-2026")
# NO OpenAI API Key needed for this bridge.
...
class AsyncBrowserThread(threading.Thread):
    ...
    async def _talk_to_chatgpt(self, prompt: str, image_path: Optional[str] = None):
        ...
        try:
            ...
            if image_path:
                print(f"[LITE-SERVER] Uploading image: {image_path}")
                # Click the attach button (usually it's an <input type="file"> or a button that triggers it)
                # ChatGPT UI uses a hidden file input
    ...
    def process_request(self, prompt: str, image_path: Optional[str] = None):
        if not self.ready_event.wait(timeout=30):
            raise Exception("Error From Browser")
            
        future = asyncio.run_coroutine_threadsafe(self._talk_to_chatgpt(prompt, image_path), self.loop)
...
async def responses(request: Request):
    ...
    provided_key = authorization.replace("Bearer ", "").strip() if authorization else api_key_header
...
async def extract_products(
    ...
    """
    Core endpoint for Mohammed's workflow. Extracts products from an image.
    """
    authorization = request.headers.get("authorization", "")
    api_key_header = request.headers.get("x-api-key", "")
    provided_key = authorization.replace("Bearer ", "").strip() if authorization else api_key_header
    ...
    try:
        ...
        # Save image to temp file for Playwright to upload
        ...
        system_prompt = """
        You are an AI assistant for 'GroceryAI', helping a store clerk named Mohammed in Egypt.
        Analyze the provided image and the command to extract product data.
        
        COMMAND: {command}
        
        INSTRUCTIONS:
        1. Identify the products in the image. 
        2. If the command says 'add' or implies stocking, set 'type' to 'stock_in'.
        3. If the command is empty or implies a sale, set 'type' to 'checkout'.
        4. Return a JSON object with:
           - "type": "stock_in" or "checkout"
           - "products": [ {"name": str, "name_ar": str, "quantity": int, "unit_price": float} ]
        5. Use Arabic for 'name_ar' and English for 'name'.
        ...
        try:
            response_text = browser_engine.process_request(system_prompt, image_path=tmp_path)
            
            # Use the existing parse_tool_calls logic or a similar regex to find the JSON
            # ChatGPT often wraps JSON in code blocks
            json_match = re.search(r'\{[\s\S]*\}', response_text)
            if json_match:
                result = json.loads(json_match.group(0))
                return result
            else:
                return JSONResponse(status_code=500, content={"error": "Failed to extract JSON from AI response", "raw": response_text})
        finally:
